...
function loadNextInQueue() {
  const doIt = () => {
    ...
    setTimeout(() => {
      player.unMute();
      player.setVolume(55); // Al reanudar, 75%
    }, 200);
...
function playCuna() {
  ...
  fadeVolume(player, 55, 12, 1000, () => {
    ...
    cunaAudio.addEventListener('ended', () => {
      ...
      fadeVolume(player, 12, 55, 1000, () => {
        scheduleNextCuna();
      });
    });
    cunaAudio.addEventListener('error', () => {
      if (!isPlaying) return;
      fadeVolume(player, 13, 55, 1000, () => {
        scheduleNextCuna();
      });
    });
  });
    ...
    .then(data => {
      if (data.items && data.items.length > 0 && data.items[0].id.videoId) {
        ...
        const doIt = () => {
          ...
          setTimeout(() => {
            player.unMute();
            player.setVolume(55); // Al reproducir related, volumen 75%
          }, 200);
